...
/// This trait represents an element of a cryptographic group with a large
/// prime-order subgroup and a comparatively-small cofactor.
...
{
    /// The large prime-order subgroup in which cryptographic operations are
    /// performed. If `Self` implements `PrimeGroup`, then `Self::Subgroup`
    /// may be `Self`.
    ...
    /// some `k`-multiple of the cofactor.
    ///
    /// The value `k` does not vary between inputs for a given implementation,
    /// but may vary between different implementations of `CofactorGroup`
    /// because some groups have more efficient methods of clearing the
    /// cofactor when `k` is allowed to be different than `1`.
    ///
    ...
    ///
    /// Returns:
    /// - `true` if `self` has trivial torsion and is in the prime-order
    ///   subgroup.
    /// - `false` if `self` has non-zero torsion component and is not in the
    ///   prime-order subgroup.
