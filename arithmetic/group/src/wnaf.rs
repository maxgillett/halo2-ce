...
/// Replaces the contents of `wnaf` with the w-NAF representation of a
...
/// A "w-ary non-adjacent form" scalar multiplication (also known as
...
///
/// For this pattern, you create a `Wnaf` context, load the scalar into it, and
/// then process each base in turn:
///
/// ```ignore
/// use group::Wnaf;
///
...
///
/// For this pattern, you create a `Wnaf` context, load the base into it, and
/// then process each scalar in turn:
///
/// ```ignore
/// use group::Wnaf;
///
...
///
/// Say you have `n` bases and `m` scalars, and want to produce `n * m` results.
/// For this pattern, you need to cache the w-NAF tables for the bases and then
/// compute the w-NAF form of the scalars on the fly for every base, or vice
/// versa:
///
/// ```ignore
/// use group::Wnaf;
...
/// ```
///
/// Alternatively, use the [`WnafBase`] and [`WnafScalar`] types, which enable
/// the various tables and w-NAF forms to be cached individually per base and
/// scalar. These types can then be directly multiplied without any additional
/// runtime work, at the cost of fixing a specific window size (rather than
/// choosing the window size dynamically).
#[derive(Debug)]
...
impl<'a, G: Group + memuse::DynamicUsage> memuse::DynamicUsage for Wnaf<usize, Vec<G>, &'a [i64]> {
    fn dynamic_usage(&self) -> usize {
        // The heap memory for the scalar representation is counted in the parent
        // `Wnaf`.
...
/// A "w-ary non-adjacent form" scalar, that uses precomputation to improve the
/// speed of scalar multiplication.
///
/// # Examples
///
...
/// A fixed window table for a group element, precomputed to improve the speed
/// of scalar multiplication.
///
/// This struct is designed for usage patterns that have long-term cached bases
/// and/or scalars, or [Cartesian products] of bases and scalars. The [`Wnaf`]
/// API enables one or the other to be cached, but requires either the base
/// window tables or the scalar w-NAF forms to be computed repeatedly on the
/// fly, which can become a significant performance issue for some use cases.
///
/// `WnafBase` and [`WnafScalar`] enable an alternative trade-off: by fixing the
/// window size at compile time, the precomputations are guaranteed to only
/// occur once per base and once per scalar. Users should select their window
/// size based on how long the bases are expected to live; a larger window size
/// will consume more memory and take longer to precompute, but result in faster
...
/// ```
///
/// Note that this pattern requires specifying a fixed window size (unlike
/// previous patterns that picked a suitable window size internally). This is
/// necessary to ensure in the type system that the base and scalar `Wnaf`s were
/// computed with the same window size, allowing the result to be computed
/// infallibly.
#[derive(Clone, Debug)]
...
impl<G: Group, const WINDOW_SIZE: usize> WnafBase<G, WINDOW_SIZE> {
    /// Computes a window table for the given base with the specified
    /// `WINDOW_SIZE`.
