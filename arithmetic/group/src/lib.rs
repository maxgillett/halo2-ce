    ...
    + ScalarMulOwned<<Self as Group>::Scalar>
{
    ...
    /// Returns an element chosen uniformly at random from the non-identity
    /// elements of this group.
    ///
    /// This function is non-deterministic, and samples from the user-provided
    /// RNG.
...
pub trait GroupEncoding: Sized {
    ...
    ///
    /// The `Default` implementation is not required to return a valid point
    /// encoding. The bound is present to enable encodings to be constructed
    /// generically:
    ///
    /// ```
    ...
    /// Attempts to deserialize a group element, not checking if the element is
    /// valid.
    ///
    ...
    /// Converts this element into its byte encoding. This may or may not
    /// support encoding the identity.
    // TODO: Figure out how to handle identity encoding generically.
    fn to_bytes(&self) -> Self::Repr;
}

/// Affine representation of a point on an elliptic curve that has a defined
