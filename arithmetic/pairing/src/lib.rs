...
pub trait Engine: Sized + 'static + Clone + Sync + Send + core::fmt::Debug {
    ...
    /// Invoke the pairing function `G1 x G2 -> Gt` without the use of
    /// precomputation and other optimizations.
...
/// Represents results of a Miller loop, one of the most expensive portions of
/// the pairing function.
///
/// `MillerLoopResult`s cannot be compared with each other until
/// [`MillerLoopResult::final_exponentiation`] is called, which is also
/// expensive.
    ...
    + for<'a> AddAssign<&'a Self>
{
    ...
    /// This performs a "final exponentiation" routine to convert the result of
    /// a Miller loop into an element of [`MillerLoopResult::Gt`], so that
