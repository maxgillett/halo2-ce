...
/// A trait that exposes additional operations related to calculating square
/// roots of prime-order finite fields.
pub trait SqrtRatio: ff::PrimeField {
    ...
    /// Computes:
    ///
    /// - $(\textsf{true}, \sqrt{\textsf{num}/\textsf{div}})$, if $\textsf{num}$
    ///   and $\textsf{div}$ are nonzero and $\textsf{num}/\textsf{div}$ is a
    ///   square in the field;
    /// - $(\textsf{true}, 0)$, if $\textsf{num}$ is zero;
    /// - $(\textsf{false}, 0)$, if $\textsf{num}$ is nonzero and $\textsf{div}$
    ///   is zero;
    /// - $(\textsf{false}, \sqrt{G_S \cdot \textsf{num}/\textsf{div}})$, if
    ///   $\textsf{num}$ and $\textsf{div}$ are nonzero and
    ///   $\textsf{num}/\textsf{div}$ is a nonsquare in the field;
    ///
    /// where $G_S$ is a non-square.
    ///
    /// For `pasta_curves`, $G_S$ is currently
    /// [`ff::PrimeField::root_of_unity`], a generator of the order $2^S$
    /// subgroup. Users of this crate should not rely on this
    /// generator being fixed; it may be changed in future crate versions to
    /// simplify the implementation of the SSWU hash-to-curve algorithm.
    ///
...
impl<F: FieldExt> SqrtHasher<F> {
    ...
    fn hash(&self, x: &F) -> usize {
        ...
        // possibly work. The 32 low-order bits are unique within the 2^S order
        // subgroup, then the xor acts as "salt" to injectively randomize the
        // output when taken modulo `hash_mod`. Since the table is small, we do
        // not need anything more complicated.
...
impl<F: FieldExt> SqrtTables<F> {
    ...
    /// Computes:
    ///
    /// * (true,  sqrt(num/div)),                 if num and div are nonzero and
    ///   num/div is a square in the field;
    /// * (true,  0),                             if num is zero;
    /// * (false, 0),                             if num is nonzero and div is
    ///   zero;
    /// * (false, sqrt(ROOT_OF_UNITY * num/div)), if num and div are nonzero and
    ///   num/div is a nonsquare in the field;
    ///
    /// where ROOT_OF_UNITY is a generator of the order 2^n subgroup (and
    /// therefore a nonsquare).
    ///
    /// The choice of root from sqrt is unspecified.
    pub fn sqrt_ratio(&self, num: &F, div: &F) -> (Choice, F) {
        ...
        //
        // We need to calculate uv and v, where v = u^((T-1)/2), u = num/div, and p-1 =
        // T * 2^S. We can rewrite as follows:
        //
        //      v = (num/div)^((T-1)/2)
        ...
        // The overall cost of this part is similar to a single full-width
        // exponentiation, regardless of S.
    ...
    /// Common part of sqrt_ratio and sqrt_alt: return their result given v =
    /// u^((T-1)/2) and uv = u * v.
...
pub trait Field64: ff::Field + ff::PrimeField + FieldExt {
    const ORDER: u64;
...
pub trait Extendable<const D: usize>: FieldExt + Sized {
    type BaseField: FieldExt;
    type Extension: FieldExt + FieldExtension<D> + From<Self>;
}

pub trait FieldExtension<const D: usize>: Sized {
    const NUM_BASE_ELEMENTS: usize = D;
