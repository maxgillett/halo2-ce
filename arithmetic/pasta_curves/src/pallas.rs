...
fn test_iso_map() {
    ...
    // This is a regression test (it's the same input to iso_map as for
    // hash_to_curve with domain prefix "z.cash:test", Shake128, and input
    // b"hello"). We don't implement Shake128 any more but that's fine.
...
fn test_hash_to_curve() {
    ...
    // This test vector is chosen so that the first map_to_curve_simple_swu takes
    // the gx1 square "branch" and the second takes the gx1 non-square "branch"
    // (opposite to the Vesta test vector).
