...
///
/// `I: IntoIterator<Item = &'a mut F: Field + ConstantTimeEq>` implements this
/// trait when the `alloc` feature flag is enabled.
///
/// For non-allocating contexts, see the [`BatchInverter`] struct.
#[cfg(feature = "alloc")]
#[cfg_attr(docsrs, doc(cfg(feature = "alloc")))]
pub trait BatchInvert<F: Field> {
    /// Consumes this iterator and inverts each field element (when nonzero).
    /// Zero-valued elements are left as zero.
    ///
...
impl BatchInverter {
    /// Inverts each field element in `elements` (when nonzero). Zero-valued
    /// elements are left as zero.
    ///
    /// - `scratch_space` is a slice of field elements that can be freely
    ///   overwritten.
    ///
    /// Returns the inverse of the product of all nonzero field elements.
    ///
    ...
    /// Inverts each field element in `items` (when nonzero). Zero-valued
    /// elements are left as zero.
    ///
    /// - `element` is a function that extracts the element to be inverted from
    ///   `items`.
    /// - `scratch_space` is a function that extracts the scratch space from
    ///   `items`.
    ///
    /// Returns the inverse of the product of all nonzero field elements.
