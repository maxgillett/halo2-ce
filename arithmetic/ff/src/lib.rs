    ...
    + for<'a> SubAssign<&'a Self>
{
    ...
    /// # Security
    ///
    /// This method provides **no** constant-time guarantees. Implementors of
    /// the `Field` trait **may** optimise this method using
    /// non-constant-time logic.
...
pub trait PrimeField: Field + From<u64> {
    ...
    /// element of this prime field, failing if the input is not canonical
    /// (is not smaller than the field's modulus).
    ///
    /// The byte representation is interpreted with the same endianness as
    /// elements returned by [`PrimeField::to_repr`].
    ...
    ///
    /// This method provides **no** constant-time guarantees. Implementors of
    /// the `PrimeField` trait **may** optimise this method using
    /// non-constant-time logic.
    ...
    /// Generic encodings of field elements should be treated as opaque.
    ...
    /// Returns a fixed multiplicative generator of `modulus - 1` order. This
    /// element must also be a quadratic nonresidue.
    ///
    /// It can be calculated using [SageMath] as
    /// `GF(modulus).primitive_element()`.
    ///
    /// Implementations of this method MUST ensure that this is the generator
    /// used to derive `Self::root_of_unity`.
    ///
    /// [SageMath]: https://www.sagemath.org/
    fn multiplicative_generator() -> Self;

    /// An integer `s` satisfying the equation `2^s * t = modulus - 1` with `t`
    /// odd.
    ///
    /// This is the number of leading zero bits in the little-endian bit
    /// representation of `modulus - 1`.
    const S: u32;

    /// Returns the `2^s` root of unity.
    ///
    /// It can be calculated by exponentiating `Self::multiplicative_generator`
    /// by `t`, where `t = (modulus - 1) >> Self::S`.
...
pub trait PrimeFieldBits: PrimeField {
    ...
    /// Converts an element of the prime field into a little-endian sequence of
    /// bits.
    fn to_le_bits(&self) -> FieldBits<Self::ReprBits>;

    /// Returns the bits of the field characteristic (the modulus) in
    /// little-endian order.
